...
const entries = `fdceba bafdgc abeg afbdgec gbeacd abced bgc fcdge bg bedgc | bafdec cgefd gcebd ebcgd
...
//////////////////////
// Part 1
const digitCounter = function (entriesArr) {
  let countDigits = 0;

  for (const entry of entriesArr) {
    const output = entry[1];
    for (let i = 0; i < output.length; i++) {
      if (String(output[i].length).match(/2|3|4|7/)) {
        countDigits++;
      }
    }
  }
...
// digitCounter(entriesArr);

const decodeEntries = function (entriesArr) {
  let total = [];
  for (let entry = 0; entry < entriesArr.length; entry++) {
    const numberBasedOnLength = (length) =>
      entriesArr[entry][0].find((e) => e.length === length);

    let numbers = new Array(9).fill(0);
    let mapping = new Array(7).fill(0);
    numbers[1] = numberBasedOnLength(2);
    numbers[4] = numberBasedOnLength(4);
    numbers[7] = numberBasedOnLength(3);
    numbers[8] = "abcdefg";

    mapping[0] = numbers[7]
      .split("")
      .map(function (e) {
        return numbers[1].indexOf(e) !== -1 ? null : e;
      })
      .join(""); // verschil tussen 7 en 1

    numbers[9] = entriesArr[entry][0].find(
      (e) =>
        e.length === 6 &&
        numbers[4].split("").every((letter) => e.split("").includes(letter))
    );

    numbers[6] = entriesArr[entry][0].find(
      (e) =>
        e.length === 6 &&
        e !== numbers[9] &&
        (e.indexOf(numbers[1].split("")[0]) !== -1) ^
          (e.indexOf(numbers[1].split("")[1]) !== -1)
    );

    numbers[0] = entriesArr[entry][0].find(
      (e) => e.length === 6 && e !== numbers[9] && e !== numbers[6]
    );

    mapping[2] = numbers[1]
      .split("")
      .map(function (e) {
        return numbers[6].indexOf(e) !== -1 ? null : e;
      })
      .join("");
    ...
    mapping[5] = numbers[1]
      .split("")
      .map(function (e) {
        return mapping[2].indexOf(e) !== -1 ? null : e;
      })
      .join("");

    numbers[3] = entriesArr[entry][0].find(
      (e) =>
        e.length === 5 &&
        e.indexOf(mapping[2]) !== -1 &&
        e.indexOf(mapping[5]) !== -1
    );

    numbers[2] = entriesArr[entry][0].find(
      (e) =>
        e.length === 5 &&
        e.indexOf(mapping[2]) !== -1 &&
        e.indexOf(mapping[5]) === -1
    );
    numbers[5] = entriesArr[entry][0].find(
      (e) =>
        e.length === 5 &&
        e.indexOf(mapping[2]) === -1 &&
        e.indexOf(mapping[5]) !== -1
    ...
    mapping[4] = numbers[6]
      .split("")
      .map(function (e) {
        return numbers[9].indexOf(e) !== -1 ? null : e;
      })
      .join("");
    ...
    for (let x = 0; x < entriesArr[entry][1].length; x++) {
      for (let i = 0; i < numbers.length; i++) {
        if (
          numbers[i].length === entriesArr[entry][1][x].length &&
          numbers[i]
            .split("")
            .every((letter) =>
              entriesArr[entry][1][x].split("").includes(letter)
            )
        ) {
          digits.push(i);
        }
    ...
    total.push(Number(digits.join("")));
  }
  console.log(total.reduce((a, b) => a + b));
};

decodeEntries(entriesArr);
